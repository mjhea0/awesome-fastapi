...
- [FastAPI Cachette](https://github.com/aekasitt/fastapi-cachette) - A light-weight opinionated caching extension to FastAPI web framework; Can be configured to a variety of key-value store backends such as Redis, Memcached, DynamoDB, MongoDB or simply store key-value pairs In-Memory. Also supports multiple Codecs for encoding/decoding bytes of data stored, and implement popular options for DataFrames.
